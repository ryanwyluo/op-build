...
REDDRAGON_XML_INSTALL_IMAGES = YES
REDDRAGON_XML_INSTALL_TARGET = YES
...
REDDRAGON_BIOS_XML_CONFIG_FILE = $(MRW_SCRATCH)/$(BR2_REDDRAGON_BIOS_XML_FILENAME)
...
define REDDRAGON_XML_BUILD_CMDS
        # copy the reddragon xml where the common lives
        bash -c 'mkdir -p $(MRW_SCRATCH) && cp -r $(@D)/* $(MRW_SCRATCH)'

        # generate the system mrw xml
        perl -I $(MRW_HB_TOOLS) \
        $(MRW_HB_TOOLS)/processMrw.pl -x $(MRW_SCRATCH)/reddragon.xml
        
        # merge in any system specific attributes, hostboot attributes
        $(MRW_HB_TOOLS)/mergexml.sh $(MRW_SCRATCH)/$(BR2_REDDRAGON_SYSTEM_XML_FILENAME) \
            ...
            $(MRW_SCRATCH)/$(BR2_REDDRAGON_MRW_XML_FILENAME) > $(MRW_HB_TOOLS)/temporary_hb.hb.xml;
        ...
        $(MRW_HB_TOOLS)/xmltohb.pl  \
            ...
            --bios-xml-file=$(REDDRAGON_BIOS_XML_CONFIG_FILE) \
...
define REDDRAGON_XML_INSTALL_IMAGES_CMDS
        mv $(MRW_HB_TOOLS)/targeting.bin $(MRW_HB_TOOLS)/$(BR2_OPENPOWER_TARGETING_BIN_FILENAME)
endef

define REDDRAGON_XML_INSTALL_TARGET_CMDS
